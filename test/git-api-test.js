...
describe('Testing GIT API', async () => {
    before((done) => {
        axios.post(`${BASE_PATH}/${REPO_NAME}`, {
            url: REPO_URL
        }).then(response => {
            assert.deepEqual(response.data, {message: 'OK', id: REPO_NAME});
        }).then(done, done).catch()
    });

    after((done) => {
        axios.delete(`${BASE_PATH}/${REPO_NAME}`).then(response => {
            assert.deepEqual(response.data, {message: 'OK'});
        }).then(done, done).catch();
    });

    it('Tesing GET all repositories from folder', (done) => {
        axios.get(`${BASE_PATH}`).then(response => {
    ...
    it('Tesing GET content from repo root', (done) => {
        axios.get(`${BASE_PATH}/${REPO_NAME}`).then(response => {
            assert.deepEqual(response.data, [
                ".babelrc",
                ".editorconfig",
                ".eslintignore",
    ...
    it('Tesing GET content from branch and path', (done) => {
        axios.get(`${BASE_PATH}/${REPO_NAME}/tree/master/src`).then(response => {
            done();
            assert.deepEqual(response.data, [
                "default-sort.js",
                "escapeRegExp.js",
    ...
    it('Tesing GET content of the file', (done) => {
        axios.get(`${BASE_PATH}/${REPO_NAME}/tree/master/src/escapeRegExp.js`).then(response => {
            done();
            assert.deepEqual(response.data, [
                "/**",
